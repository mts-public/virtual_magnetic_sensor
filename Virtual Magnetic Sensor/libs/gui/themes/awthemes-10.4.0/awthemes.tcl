...
#     Sets the bars colors and also changes any checkbutton and
#     radiobutton types to use thematic images.
#     Side effect: The bars will have -hidemargin set to true.
#
...
#   - Menus: add support for bars foreground (bars.fg).
#   - Option database initialization: Do not initialize the bars colors
#     on Windows.  Using 'setMenuColors' on Windows leaves the top menubar
#     a light color, and the bars colors dark with a large border.
...
#   - Menus: add support for bars relief (bars.relief).  Default to 'raised'.
...
# 8.0 (2020-4-18)
#   - bars radiobuttons and bars checkbuttons are now dynamically generated
#     and any corresponding .svg files have been removed.
#     This also fixes bars radio/check button sizing issues for themes
...
#   - adjust sizing for bars checkbutton and radiobutton.
...
namespace eval ::ttk::awthemes {
  ...
  proc init { theme } {
    ...
    set vars(cache.bars) [dict create]
...
#END-PNG-DATA
  ...
  proc setMenuColors { w {theme {}} } {
    ...
    # the standard bars does not have a -mode option
    ...
    if { $w eq "-optiondb" } {
      option add *Menu.background $colors(bars.bg)
      option add *Menu.foreground $colors(bars.fg)
      ...
      option add *Menu.selectColor $colors(bars.fg)
      option add *Menu.relief $colors(bars.relief)
      # don't scale the borderwidth here
      option add *Menu.borderWidth 1
      return
    }

    if { ! [dict exists $vars(cache.bars) $w] } {
      dict set vars(cache.bars) $w 1
    }

    $w configure -background $colors(bars.bg)
    $w configure -foreground $colors(bars.fg)
    ...
    $w configure -selectcolor $colors(bars.fg)
    $w configure -relief $colors(bars.relief)
    # don't scale the borderwidth here
...
namespace eval ::themeutils {
  ...
  proc init {} {
    ...
    set vars(names.colors.derived) { \
      ...
      other     bars.relief                   raised                  static
