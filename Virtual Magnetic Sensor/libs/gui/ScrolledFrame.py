...
Original by Benjamin Johson.
Slightly modified to allow changes of the canvas background color.
"""
...
class ScrolledFrame(tk.Frame):
    ...
    def __init__(self, master=None, **kw):
        ...
        c = self._canvas = tk.Canvas(self,
                                     ...
                                     takefocus=0)
        if "background" in kw:
            self._canvas.configure(background=kw["background"], highlightbackground=kw["background"])
        if "bg" in kw:
            self._canvas.configure(background=kw["bg"], highlightbackground=kw["bg"])
