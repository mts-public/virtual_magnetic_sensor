...
class CSGEvoKF:
    ...
    def evokf_2d_point_array(self, involute_points: int, c: float, tooth_side: str) -> np.array:
        ...
        tooth_side_coord = np.append(tooth_side_coord, np.array([[tooth_side_coord[-1, 0]], [self.CSGEvoGear_cls.EvoTooth_ini.d_f/2*0.99]]).T, axis=0)

        v = np.reshape(tooth_side_coord[-1]-tooth_side_coord[0], (2, 1))
        v = c*(1/(np.sqrt(np.power(v[0], 2)+np.power(v[1], 2))))*v
        v_ = np.dot(self.CSGEvoGear_cls.EvoTooth_ini.rotation_matrix(np.radians(-90))[0:2, 0:2], v)
        ...
        for i in range(1, tooth_side_coord.shape[0]):
            if i < tooth_side_coord.shape[0]-1:
                m_int = (tooth_side_coord[i, 1]-tooth_side_coord[i-1, 1])/(tooth_side_coord[i, 0]-tooth_side_coord[i-1, 0])
                x = (tooth_side_coord[i-1, 1]-diff[0, 1]+m_c * diff[0, 0]-m_int*tooth_side_coord[i-1, 0])/(m_c-m_int)
        ...
        if len(indices) == 2:
            for i in range(sol.shape[0]):
                tooth_side_coord = np.insert(tooth_side_coord, int(sol[i, 2]), np.reshape(sol[i, [0, 1]], (1, 2)), axis=0)
            ...
            match tooth_side:
                case "right":
                    tooth_side_coord = np.vstack((self.CSGEvoGear_cls.evotooth_2dpoint_array(involute_points)[1], tooth_side_coord))
                    tooth_side_coord = np.vstack((tooth_side_coord, [(-1)*tooth_side_coord[-1, 0], self.CSGEvoGear_cls.EvoTooth_ini.d_f/2*0.99]))
                    return tooth_side_coord
                case "left":
                    lst_entry_ts_coord=np.copy(tooth_side_coord[-1])
                    tooth_side_coord=tooth_side_coord[0:-1,::]
                    tooth_side_coord[::,0]=tooth_side_coord[::-1,0]*(-1)
                    tooth_side_coord[::,1]=tooth_side_coord[::-1,1]
                    tooth_side_coord = np.vstack((tooth_side_coord,self.CSGEvoGear_cls.evotooth_2dpoint_array(involute_points)[2][::-1]))
                    tooth_side_coord = np.vstack((tooth_side_coord,lst_entry_ts_coord))
                    tooth_side_coord = np.vstack((tooth_side_coord, [(-1)*tooth_side_coord[-1, 0], self.CSGEvoGear_cls.EvoTooth_ini.d_f/2*0.99]))
                    return tooth_side_coord
        elif len(indices)==1:
            for index,entry in enumerate(tooth_side_coord):
                if index > 0:
                    v = np.reshape(tooth_side_coord[index]-tooth_side_coord[index-1], (2, 1))
                    v = c*(1/(np.sqrt(np.power(v[0], 2)+np.power(v[1], 2))))*v
                    v_ = np.dot(self.CSGEvoGear_cls.EvoTooth_ini.rotation_matrix(np.radians(-90))[0:2, 0:2], v)
                    v_ = np.reshape(v_, (1, 2))
                    tooth_side_coord[index] = tooth_side_coord[index]+v_
                    if index==1:
                        tooth_side_coord[0,0]=tooth_side_coord[0,0]+v_[0,0]
            match tooth_side:
                case "right":
                    tooth_side_coord = np.vstack((self.CSGEvoGear_cls.evotooth_2dpoint_array(involute_points)[1], tooth_side_coord))
                    return tooth_side_coord
