...
class NGMesh:
    ...
    def rotate_gear_mesh(mesh: msh.Mesh, mp: msh.MeshingParameters, data_handler: DataHandler,
        ...
        for p in mesh.Points():
            ...
            if abs(p_canonical[2]) <= data_handler.objects[idx].length/2 * (1 + 1e-2) and sqrt(
                    pow(p_canonical[0], 2) + pow(p_canonical[1], 2)) <= (
                    data_handler.objects[idx].diameter[1] + data_handler.objects[idx].tooth_height)/2 * (1 + 1e-2):
